...
function buildColoredSalarySheet(XLSX, employees, year, month, attendance, settings) {
  ...
  const headers = [
    ...
    'Paid Leaves (L)', 'Absent Deductions (A)',
  ...
  employees.forEach((emp, ri) => {
    ...
    const vals = [
      ...
      c.paidLeaves, c.unpaidAbsent,
      c.perDay, c.effectiveDays, c.deduction, c.netSalary,
    ];
    const rowBg = ri % 2 === 0 ? BG.WHITE : 'F5F5F5';
    vals.forEach((v, ci) => {
      const isCurrency = [3, 12, 14, 15].includes(ci);
      ws[XLSX.utils.encode_cell({ r: row, c: ci })] = {
        v,
        t: typeof v === 'number' ? 'n' : 's',
        s: {
          fill: { fgColor: { rgb: ci === 14 && c.deduction > 0 ? 'FFC7CE' : ci === 15 ? 'C6EFCE' : rowBg } },
          font: { sz: 10, bold: [1, 15].includes(ci) },

...
export function isWeekOff(date, settings) {
  ...
  // Explicit exceptions override everything → forced working day
  if ((settings?.weekOffExceptions ?? []).includes(dateNum)) return false;

  // Custom week-offs for any specific date → forced week-off
  if ((settings?.customWeekOffs ?? []).includes(dateNum)) return true;

  // Default Sunday rule
  if (day === 0) return settings?.allSundaysOff ?? true;

  // Default Saturday rule
  if (day === 6) {
